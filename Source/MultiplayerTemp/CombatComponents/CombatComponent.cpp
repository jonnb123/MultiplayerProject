...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			// calculate crosshair spread. need to convert move speed (i.e. value from 0 to 600) to range (0,1)
			// i.e [0, 600] -> [0,1]
			...
			{
				CrosshairInAirFactor = FMath::FInterpTo(CrosshairInAirFactor, 2.3, DeltaTime, 2.3);
			}
			else
			{
				CrosshairInAirFactor = FMath::FInterpTo(CrosshairInAirFactor, 0.f, DeltaTime, 32.f);
			}
			if (bAiming)
			{
				CrosshairAimFactor = FMath::FInterpTo(CrosshairAimFactor, 0.6f, DeltaTime, 32.f);
			}
			else
			{
				CrosshairAimFactor = FMath::FInterpTo(CrosshairAimFactor, 0.f, DeltaTime, 32.f);
			}
...
void UCombatComponent::FireButtonPressed(bool bPressed)
{
	...
	// remember calling serverfire() calls serverfire_Implementation
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	// to get crosshair location to center of viewport, these are currently in screenspace, need them in worldspace
